...
- **OOF 계약 = 디커플링 경계**: 모든 모델은 동일 fold(seed)로 `experiments/oof/<exp_id>.csv`=`[id, oof]`, `submissions/<exp_id>.csv`=`[id, <target>]` 만 산출한다. `src/stack.py` 는 **이 계약만** 소비 → 멤버가 늘어도 스택에 모델별 특수 코드가 0. ⚠️ multiclass 는 OOF 가 K개 확률 열(`oof_<label>`)·제출은 단일 예측 라벨이며, 스택용 멤버 test 확률은 `experiments/test_pred/<exp>.csv`=`[id, <label>...]` K열로 별도 저장한다. 스태킹도 multiclass 지원(`equal`/`multinomial`/`nnls`).
