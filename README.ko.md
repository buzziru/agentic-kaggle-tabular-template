...
- **이미지/NLP/딥러닝 중심** 과제다 → 스캐폴드는 **GBDT/타뷸라 우선**(LightGBM/XGBoost 어댑터)이다.
...
| 모델 추가 = 어댑터 (스캐폴드 복제 금지)                    | `src/train_common.py` + `src/registry.py` + `src/train_<model>.py` (진입점 `src/train.py`) |
