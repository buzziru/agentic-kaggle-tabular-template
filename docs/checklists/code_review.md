...
>
> **적용 범위(리스크 티어)**: 공유 스캐폴드(`features.py`/`train_common.py`/`cv.py`/`encoders.py`)·새 모델 어댑터·비자명 로직 변경에만 필수. config-only·문서·패스스루 베이스라인은 면제(단 frozen-OOF/divergence 위험 시 필수).
